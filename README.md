...
- **TG 凭证**：Telegram Bot Token / Chat ID
- **TG 汇报时间** `HH:MM:SS`：默认 `20:00:00`。Worker 每 15 分钟检查，到点自动发 TG 汇总（也可点「立即汇报」手动发）
- **TG 汇报模板**：内置「简洁日报 / 详细日报」两个模板，可编辑、新建自定义、删除、恢复内置；支持用当前数据「预览」。占位符见页面提示
- **CF 上报 cron**：VPS 端默认上报频率，默认 `0 * * * *`

> 机器列表支持**多选批量操作**：勾选多台后可批量删除、加入 TG 汇报、移出 TG 汇报。只有「加入 TG 汇报」的机器才会进入汇总。
...
- 列表支持「只看在线」筛选 + 按今日/本月流量/累积在线排序；每行 **流量统计**（单机弹窗）/ **更新注册** / **删除**
