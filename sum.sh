...
# TG 时间：HH:MM:SS，例如 20:00:00
validate_time() { [[ "$1" =~ ^([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$ ]]; }
# CF 调度：5 段 cron，例如 0 * * * * / 0 */6 * * *
validate_cron() {
  local f1 f2 f3 f4 f5
  [[ "$1" =~ ^[[:space:]]*([^[:space:]]+)[[:space:]]+([^[:space:]]+)[[:space:]]+([^[:space:]]+)[[:space:]]+([^[:space:]]+)[[:space:]]+([^[:space:]]+)[[:space:]]*$ ]] || return 1
  f1="${BASH_REMATCH[1]}"; f2="${BASH_REMATCH[2]}"; f3="${BASH_REMATCH[3]}"
  f4="${BASH_REMATCH[4]}"; f5="${BASH_REMATCH[5]}"
  ...
  [[ "${f5}" =~ ^(\*|\*/[0-9]+|[0-7]+(-[0-7]+)?(,[0-7]+(-[0-7]+)?)*)(/[0-9]+)?$ ]] || return 1
  return 0
}
...
# 环境变量 ttime：TG 汇报时间，默认 20:00:00
resolve_ttime() {
...
# 环境变量 cftime：CF 汇报 cron；空则禁用 CF
# 示例：0 * * * *（每小时整点）、0 */6 * * *（每 6 小时）
resolve_cftime() {
  local val="${cftime:-}"
  if [[ -z "${val}" ]]; then
    printf ''
...
# CF 可选：cfurl / cftoken / mid；配置了 cftime 时必填
resolve_cf_opts() {
  local url="${cfurl:-}" token="${cftoken:-}" id="${mid:-}"
  CF_URL="${url}"
  CF_TOKEN="${token}"
  MACHINE_ID="${id}"
  if [[ -z "${cftime:-}" ]]; then
  ...
  [[ -n "${url}" ]] || die '已设置 cftime，请同时设置 cfurl（Worker 上报地址）。'
  [[ -n "${token}" ]] || die '已设置 cftime，请同时设置 cftoken（上报鉴权令牌）。'
  [[ -n "${id}" ]] || die '已设置 cftime，请同时设置 mid（机器 ID，如 hk-1）。'
  validate_url "${url}" || die 'cfurl 须为 https:// 开头的有效 URL。'
  validate_cftoken "${token}" || die 'cftoken 格式无效（8-256 位字母数字及 _.-+/~）。'
  validate_mid "${id}" || die 'mid 格式无效（1-64 位字母数字及 ._: -）。'
}

# 常见 5 段 cron → systemd OnCalendar（覆盖日常场景）
cron_to_oncalendar() {
  local cron="$1"
  ...
  # 星期：cron 0/7=Sun → systemd Sun..Sat
  local day_part=""
  case "${dow}" in
    \*) day_part="" ;;
    0|7) day_part="Sun " ;;
    ...
    *)
      ...
      if [[ "${dow}" =~ ^[0-7](,[0-7])*$ ]]; then
        ...
        die "暂不支持的 cron 星期字段：${dow}（可用 * / 0-6 / 1-5）"
      fi
      ;;
  esac

  [[ "${dom}" == "*" ]] || die "暂不支持按日字段非 * 的 cron：${cron}"
  [[ "${mon}" == "*" ]] || die "暂不支持按月字段非 * 的 cron：${cron}"
  ...
  if [[ "${min}" == "*" ]]; then
    ...
    m_part="0/${BASH_REMATCH[1]}"
  elif [[ "${min}" =~ ^[0-9]+$ ]]; then
    m_part="$(printf '%02d' "$((10#${min}))")"
  else
    die "暂不支持的 cron 分钟字段：${min}"
  fi

  # OnCalendar: [Day] *-*-* HH:MM:SS  或  *-*-* *:MM:SS / 0/6:00:00
  if [[ "${h_part}" == "*" ]]; then
    printf '%s*-*-* *:%s:00' "${day_part}" "${m_part}"
  elif [[ "${h_part}" =~ / ]]; then
    # 0/6 → 每 6 小时从 0 点起
...
write_config() {
  local ifname="$1" token="$2" chat_id="$3" cf_url="$4" cf_token="$5" machine_id="$6"
  local tmp
...
send_cf() {
  local payload resp
  [[ -n "${CF_URL}" ]] || { log_error "未配置 CF_URL，跳过 CF 上报。"; return 1; }
  [[ -n "${CF_TOKEN}" ]] || { log_error "未配置 CF_TOKEN。"; return 1; }
  [[ -n "${MACHINE_ID}" ]] || { log_error "未配置 MACHINE_ID。"; return 1; }
  ...
  resp="$(curl --silent --show-error --fail-with-body \
    ...
    --data "${payload}" \
    "${CF_URL}")" || {
    log_error "CF 上报失败。"; return 1; }

  if jq -e 'type == "object"' >/dev/null 2>&1 <<<"${resp}"; then
    jq -e '.ok != false' >/dev/null 2>&1 <<<"${resp}" || {
      log_error "CF 返回失败：$(jq -r '.error // .message // "未知错误"' <<<"${resp}")"
      return 1
    }
  fi
  return 0
...
main() {
  ...
  case "${1:-}" in
    ...
    --cf)        run_cf ;;
    "")          run_tg ;;
    *)           log_error "未知参数：${1}（可用 --tg / --cf / --test）"; return 1 ;;
  esac
}
...
[Service]
Type=oneshot
ExecStart=/usr/local/sbin/traffic-telegram-report --tg
User=root
Group=root
UMask=0077
...
[Unit]
Description=Send vnStat traffic report to Cloudflare Worker
After=network-online.target vnstat.service
Wants=network-online.target vnstat.service

[Service]
Type=oneshot
ExecStart=/usr/local/sbin/traffic-telegram-report --cf
User=root
Group=root
UMask=0077
NoNewPrivileges=true
...
enable_timers() {
  ...
  if [[ "${enable_cf}" == "1" ]]; then
    ...
    rm -f "${CF_TIMER_FILE}" "${CF_SERVICE_FILE}"
    systemctl daemon-reload
  fi
}
...
send_test() {
  ...
  "${REPORT_SCRIPT}" --test || \
    die 'Telegram 测试发送失败。请确认 Bot 已加入目标会话，且 Chat ID 正确。'
  if [[ "${enable_cf}" == "1" ]]; then
    log '发送 Cloudflare 测试上报...'
    "${REPORT_SCRIPT}" --cf || \
      die 'Cloudflare 测试上报失败。请检查 cfurl / cftoken / mid 与 Worker。'
  fi
}

print_summary() {
  local ifname="$1" tg_time="$2" cf_cron="$3"
  printf '\n安装完成。\n'
  printf '  监控网卡：  %s\n'   "${ifname}"
  printf '  TG 汇报：   每天 %s（服务器本地时区）\n' "${tg_time}"
  if [[ -n "${cf_cron}" ]]; then
    printf '  CF 汇报：   cron %s → %s\n' "${cf_cron}" "$(cron_to_oncalendar "${cf_cron}")"
    printf '  机器 ID：   %s\n' "${MACHINE_ID}"
    printf '  CF 地址：   %s\n' "${CF_URL}"
  else
    printf '  CF 汇报：   未启用（设置 cftime/cfurl/cftoken/mid 可开启）\n'
  fi
  printf '  配置文件：  %s（仅 root 可读）\n'  "${CONFIG_FILE}"
  printf '  查看 TG 定时：systemctl status %s.timer\n'   "${APP_NAME}"
  printf '  立即 TG 发送：systemctl start %s.service\n'  "${APP_NAME}"
  if [[ -n "${cf_cron}" ]]; then
    printf '  查看 CF 定时：systemctl status %s-cf.timer\n' "${APP_NAME}"
    printf '  立即 CF 上报：systemctl start %s-cf.service\n' "${APP_NAME}"
  fi
  printf '  查看日志：  journalctl -u %s.service -u %s-cf.service\n' "${APP_NAME}" "${APP_NAME}"
...
main() {
  local ifname token chat_id tg_time cf_cron enable_cf=0
  require_root
  ...
  resolve_cf_opts
  [[ -n "${cf_cron}" ]] && enable_cf=1

  install_deps
  ifname="$(detect_interface)"
  configure_vnstat "${ifname}"
  write_config "${ifname}" "${token}" "${chat_id}" "${CF_URL}" "${CF_TOKEN}" "${MACHINE_ID}"
  write_reporter
  write_service_unit
  ...
  enable_timers "${enable_cf}"
  send_test "${enable_cf}"
  print_summary "${ifname}" "${tg_time}" "${cf_cron}"
}
